...
- Streams real Binance spot depth, trades and candles, verified accessible from the deployment server.
...
This deployment is a **public preview**, with a Next development server. Server policy prohibits full Next builds/typechecks on yusam; run a production build on a suitable build machine before claiming production readiness. Java and Go are compiled services. Tests cover unit/domain rules, real API/LLM flows, restart recovery and desktop/mobile journeys; these are not claims of exchange-scale throughput or profitable trading.
