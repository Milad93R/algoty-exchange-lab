...
(async () => {
  ...
  try {
    ...
    await page.getByText("Converted quantity", { exact: false }).waitFor();
    ...
    await page
      .getByText("OCO accepted", { exact: false })
    ...
    await page.getByLabel("Order amount").fill("25");
    ...
    const amountMode = page.getByRole("button", { name: "Amount", exact: true });
    await amountMode.waitFor({ timeout: 30000 });
    assert.strictEqual(await amountMode.getAttribute("aria-pressed"), "true");
    assert.deepStrictEqual(errors, []);
    const evidence = {
      checks: [
        "OCO ticket exposes limit, stop trigger and stop-limit prices",
        "USDT amount input converts to an executable base-asset quantity",
        "market order entered as a USDT amount reaches FILLED",
        ...
        "amount sizing mode persists after reload",
      ],
      errors,
    };
    fs.writeFileSync(
