...
- The Exchange chart loads 500 REST candles for 1m, 5m, 15m, 30m, 1h, 4h and 1d. Each symbol/timeframe has its own TanStack Query cache; the server response is briefly shared-cached according to the interval.
...
The exchange has zoomable candles, optional SMA, depth-price selection, execution detail, positions, assets and ledger. Mobile layouts preserve the order ticket and book. Missions have editable plans, an evidence journal, natural-language control previews and read-only comparison publishing.
